...
parcel=np.full((601,34,2),np.nan)
for time in range(601):
...
for time,(index_rmw,index_outer) in enumerate(zip(rmw,outer)):
    for i,indx in enumerate([index_rmw,index_outer]):
        ...
        lfcp,lfct=metcal.lfc(pres[time,:,indx],temp[time,:,indx],Td[time,:,indx],parcel[time,:,i]*units.degC)
        lfc_p[time,i]=lfcp.magnitude
        lfc_t[time,i]=lfct.magnitude
        elp,elt=metcal.el(pres[time,:,indx],temp[time,:,indx],Td[time,:,indx],parcel[time,:,i]*units.degC)
