...
for time in range(73):
    ...
    pressure_lcl, _ = metcal.lcl(pres[time,sfc], temp[time,sfc], Td[time,sfc])
    below_lcl = pres[time,sfc:] > pressure_lcl
    parcel_mixing_ratio[time,:] = np.where(below_lcl,rv[time,sfc],metcal.saturation_mixing_ratio(pres[time,sfc:], parcel[time,:]*units.degC))
    Tv_parcel[time,:]=metcal.virtual_temperature(parcel[time,:]*units.degC,parcel_mixing_ratio[time])
...
for time in range(73):
    ...
    lfcp,lfct=metcal.lfc(pres[time,sfc:],metcal.virtual_temperature(temp[time,sfc:],rv[time,sfc:]),Td[time,sfc:],Tv_parcel[time,:]*units.degC)
    lfc_p[time]=lfcp.magnitude
    lfc_t[time]=lfct.magnitude
    elp,elt=metcal.el(pres[time,sfc:],metcal.virtual_temperature(temp[time,sfc:],rv[time,sfc:]),Td[time,sfc:],Tv_parcel[time,:]*units.degC)
...
for time in range(0,73,1):
    ...
    # Plot the data
    skew.plot(pres[time,:],metcal.virtual_temperature(temp[time,:],rv[time,:]),'blue',linewidth=2)
    skew.plot(pres[time,:],Td[time,:],'red',linewidth=2)
    skew.plot(pres[time,sfc:],Tv_parcel[time,:],'k',linewidth=1.5)
    ...
    plt.tight_layout()
    plt.savefig(f'./skewTR/skewT-{time//3:02d}{time*20%60:02d}.png',dpi=450)
    plt.close(fig)
