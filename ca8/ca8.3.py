...
diff=[]
for time in range(601):
    parcel_mse[time,:]=np.tile(mse[time,0,rmw[time]],(1,34))
    diff.append(parcel_mse[time]-mses[time,:,rmw[time]])
diff=np.array(diff) 
...
for time in range(601):
    ...
    Tout.append(temp[time,el[time],rmw[time]])
    Tin.append(temp[time,0,rmw[time]])
...
qvsfc=[]
for i in range(601):
    tsfc.append(np.nanmean(temp[i,0,0:rmw[i]]))
    qvsfc.append(np.nanmean(qv[i,0,0:rmw[i]]))
tsfc=np.array(tsfc)
...
diff=[]
for time in range(601):
    parcel_mse2[time,:]=np.tile(mse2[time,0,rmw[time]],(1,34))
    diff.append(parcel_mse2[time]-mses2[time,:,rmw[time]])
diff=np.array(diff) 
...
for time in range(601):
    ...
    Tout2.append(temp2[time,el2[time],rmw[time]])
    Tin2.append(temp2[time,0,rmw[time]])
...
qvsfc2=[]
for i in range(601):
    tsfc2.append(np.nanmean(temp2[i,0,0:rmw[i]]))
    qvsfc2.append(np.nanmean(qv2[i,0,0:rmw[i]]))
tsfc2=np.array(tsfc2)
qvsfc2=np.array(qvsfc2)
...
plt.plot(np.arange(601),mpi_origin,'blue',label='Original')
...
plt.grid()
plt.legend()
plt.savefig('mpi.png',dpi=450,bbox_inches='tight')
...
plt.grid()
plt.legend()
plt.savefig('mpi_smooth.png',dpi=450,bbox_inches='tight')
...
plt.grid()
plt.legend()
plt.savefig('TC_cape.png',dpi=450,bbox_inches='tight')
...
plt.grid()
plt.savefig('diff_cape.png',dpi=450,bbox_inches='tight')
